    ...
    @objc private func registerButtonTap() {
        Task {
            do {
                ...
                //중복 체크
                let duplicateStatus = try await DuplicateService.shared.GetRegisterData(username: self.usreName)
                
                guard let isExist = duplicateStatus?.isExist else {
                    return
                }
                
                //중복할 때
                if isExist == true {
                    let alert = UIAlertController(title: "중복입니다", message: "와하하", preferredStyle: UIAlertController.Style.alert)
